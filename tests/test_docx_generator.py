...
class TestDOCXGenerator:
    ...
    def test_save_to_output_folder(self):
        ...
        gen = DOCXGenerator()
        gen.create_document("Unit Test Output")

        gen.add_heading("Test Results", level=1)
        gen.add_paragraph("This document was generated by unit tests.")
        gen.add_bullet_list([
            "Test 1: Passed",
            "Test 2: Passed",
            "Test 3: Passed",
        ...
        filepath = output_dir / "test_output_unit_test.docx"
        saved_path = gen.save(str(filepath))

        assert os.path.exists(saved_path)
        assert "test_output_unit_test.docx" in saved_path
